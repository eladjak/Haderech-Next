    ...
    <div className={`relative ${className ?? ""}`}>
      ...
      {showMenu && (
        <div
          ...
          className="absolute top-full left-0 z-50 mt-2 w-48 rounded-xl border border-brand-100/30 bg-white p-2 shadow-lg dark:border-zinc-700 dark:bg-zinc-900"
        >
          {shareLinks.map((link) => (
            <a
              key={link.name}
          ...
          >
            <span aria-hidden="true">{copied ? "\u2705" : "\u{1F4CB}"}</span>

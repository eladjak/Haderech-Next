...
export default async function CoursePage({ params }: RouteParams) {
  ...
  if (session) {
    ...
    if (isEnrolled) {
      completedLessons = typedCourse.lessons?.filter(lesson => 
    ...
    <div className="container py-8">
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-8">
          ...
          <CourseComments comments={typedCourse.comments} courseId={typedCourse.id} />
        </div>
        <div>
          <div className="sticky top-20 space-y-8">
            <CourseSidebar course={typedCourse} isEnrolled={isEnrolled} progress={progress} />
            {isEnrolled && (
              <CourseProgress completedLessons={completedLessons} totalLessons={totalLessons} />
            )}
          </div>
        </div>
      </div>

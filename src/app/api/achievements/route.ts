...
export async function POST(req: Request) {
  try {
    ...
    const existingAchievements = currentAchievements?.map(a => a.achievement_id) || []
    const newAchievements = []

    // Check each achievement condition
    if (isFirstLogin && !existingAchievements.includes(ACHIEVEMENTS.FIRST_LOGIN.id)) {
      newAchievements.push(ACHIEVEMENTS.FIRST_LOGIN.id)
    }

    if (profileCompletion === 100 && !existingAchievements.includes(ACHIEVEMENTS.COMPLETE_PROFILE.id)) {
      newAchievements.push(ACHIEVEMENTS.COMPLETE_PROFILE.id)
    }

    if (completedCourses > 0 && !existingAchievements.includes(ACHIEVEMENTS.FIRST_COURSE.id)) {
      newAchievements.push(ACHIEVEMENTS.FIRST_COURSE.id)
    }

    if (joinedGroups >= 5 && !existingAchievements.includes(ACHIEVEMENTS.SOCIAL_BUTTERFLY.id)) {
      newAchievements.push(ACHIEVEMENTS.SOCIAL_BUTTERFLY.id)
    }

    if (chatInteractions >= 50 && !existingAchievements.includes(ACHIEVEMENTS.CHAT_MASTER.id)) {
      newAchievements.push(ACHIEVEMENTS.CHAT_MASTER.id)
    }

    if (simulatorInteractions >= 50 && !existingAchievements.includes(ACHIEVEMENTS.SIMULATOR_MASTER.id)) {
      newAchievements.push(ACHIEVEMENTS.SIMULATOR_MASTER.id)
    }

    if (completedSimulations >= 10 && averageFeedback >= 4.5 && !existingAchievements.includes(ACHIEVEMENTS.FEEDBACK_CHAMPION.id)) {

...
export const socialSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentGroup: (state: SocialState, action: PayloadAction<SocialGroup | null>) => {
      state.currentGroup = action.payload
    },
    setCurrentActivity: (state: SocialState, action: PayloadAction<Activity | null>) => {
      state.currentActivity = action.payload
    },
    addActivity: (state: SocialState, action: PayloadAction<{ groupId: string; activity: Activity }>) => {
    ...
    joinActivity: (state: SocialState, action: PayloadAction<{ groupId: string; activityId: string; userId: string }>) => {
      const group = state.groups.find(g => g.id === action.payload.groupId)
      if (group) {
        const activity = group.activities.find(a => a.id === action.payload.activityId)
        if (activity && !activity.participants.includes(action.payload.userId)) {
          activity.participants.push(action.payload.userId)
        }
      }
      if (state.currentGroup && state.currentGroup.id === action.payload.groupId) {
        const activity = state.currentGroup.activities.find(a => a.id === action.payload.activityId)
        if (activity && !activity.participants.includes(action.payload.userId)) {
          activity.participants.push(action.payload.userId)
        }
      }
    },
    updateFilters: (state: SocialState, action: PayloadAction<Partial<SocialState['filters']>>) => {

...
export async function startSimulation(
  ...
  try {
    ...
    const { data: response } =
      await supabase.functions.invoke<SimulationResponse>("start-simulation", {
        body: { context },
      });
 ...
 */
async function generatePartnerResponse(
  state: SimulationState,
): Promise<SimulationState> {
  try {
    const { data: response } =
      await supabase.functions.invoke<SimulationResponse>("generate-response", {
        body: { state },
      });
    ...
    const updatedEmotionalState = await updateEmotionalState(
      state,
      response.message,
    );
...
async function updateEmotionalState(
  ...
  try {
    const { data: response } =
      await supabase.functions.invoke<SimulationResponse>("analyze-emotion", {
    ...
    return {
      ...
      interest: Math.max(
        0,
        Math.min(100, response.interest || state.emotionalState.interest),
      ),
      comfort: Math.max(
        0,
        Math.min(100, response.comfort || state.emotionalState.comfort),
      ),
    };
  } catch (error) {

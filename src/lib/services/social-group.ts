...
export const joinActivity = async (
  ...
  try {
    ...
    const activityIndex = group.activities.findIndex((a: Activity) => a.id === activityId);
    ...
    if (activity.max_participants && activity.participants.length >= activity.max_participants) {
